...
if __name__=='__main__':
    ...
    resultFiles = [
             # Result file                  , size value
            ["./models_saved/saved_EffV6_32/resultsAdv3L.pkl",   32],
            ["./models_saved/saved_EffV6_64/resultsAdv3L.pkl",   64],
            ["./models_saved/saved_EffV6_128/resultsAdv3L.pkl",  128],
            ["./models_saved/saved_EffV6_160/resultsAdv3L.pkl",  160],
            ["./models_saved/saved_EffV6_192/resultsAdv3L.pkl",  192],
            ["./models_saved/saved_EffV6_224/resultsAdv3L.pkl",  224],
            ["./models_saved/saved_EffV6_256/resultsAdv3L.pkl",  256],
            ["./models_saved/saved_EffV6_288/resultsAdv3L.pkl",  288],
            ["./models_saved/saved_EffV6_320/resultsAdv3L.pkl",  320],
            ["./models_saved/saved_EffV6_352/resultsAdv3L.pkl",  352],
            ["./models_saved/saved_EffV6_384/resultsAdv3L.pkl",  384]
        ]  
    
    count = 1
    for resultFile, size in resultFiles:
        print(resultFile, size)
    ...
    acc_level1_df =  pd.DataFrame({"Size":size_values, "Accuracy":acc_level1, "Level":'L1'})
    acc_level2_df =  pd.DataFrame({"Size":size_values, "Accuracy":acc_level2, "Level":'L2'})
    acc_level3_df =  pd.DataFrame({"Size":size_values, "Accuracy":acc_level3, "Level":'L3'})
    average_accuracy_df = pd.DataFrame({"Size":size_values, "Accuracy":acc_avg_levels, "Level":'Avg'})
    data_to_plot = pd.concat([acc_level1_df, acc_level2_df, acc_level3_df, average_accuracy_df])
    g = sns.lineplot(data=data_to_plot, x='Size', y='Accuracy', hue='Level', style='Level', markers=True)
    #g.set(xscale='log')
    #g.set(ylim=(96.5,99.5))
    plt.title('Accuracy vs. input size')
    plt.savefig(graph_folder+'accuracy_size.png')
    plt.show()    
    print(size_values, best_epoch, acc_avg_levels)
    ...
    plt.savefig(graph_folder+'accuracy_train_size.png')
    plt.show()    
    print(size_values, best_epoch, acc_avg_levels)
