# -*- coding: utf-8 -*-
"""
Created on Mon Aug 22 14:38:53 2026

Used to create dataset for detection using YOLO11 based on RGB and MIE images selected from images recorded in Sweden, Greenland and Alaska
Use camera systems from Tundra sites ('Abisko', 'Narsarsuaq', 'Toolik')
...
def createLabelsAndImages(selDataset, data_df, pathToRecordedFiles, pathToDestDataset, pathToDestDatasetMIE, split):
    ...
    for idx, row in selDataset.iterrows():
        ...
        cameraId = "TD_" + row['site'] + row['fileName'].split('/')[0] + '_'
        count += 1
...
if __name__=='__main__':
    ...
    numUnsure = 300
    numVegetation = 300
    splitPercentage = 10 # Percentage of image used for validation and test
    pathToHome = '/home/don/insectsDCT/TOKE/'
    #pathToHome = 'J:/TOKE_tundraplants/'
    pathToSrcDataset = pathToHome
    pathToRecordData = '/mnt/data0/don/TOKE/'
    pathToDestDatasetMIE = pathToHome+'/trainTDRm/'
    pathToDestDataset = pathToHome+'/trainTDR/'
    partnerIds = ['detections_Abisko_25']
    #partnerIds = ['detections_Narsarsuaq_22']
    #partnerIds = ['detections_Toolik_23']
        ...
        for filename in sorted(os.listdir(pathToSrcDetections)):
            if (filename.endswith('.csv')):
                if "-CL.csv" in filename:
                    ...
                    data_df['partner'] = '' # Abisko part of trapDir
                    data_df['site'] = ''
                    if "Narsarsuaq" in partner:
                        data_df['partner'] = "Narsarsuaq/"
                        data_df['site'] = 'NARS_'
                    if "Toolik" in partner:
                        data_df['partner'] = "Toolik/"
                        data_df['site'] = 'TOOL_'
                    if firstTime:
