...
def createHierarchicalClassifier(weights_file, label_file, threshold_file, img_size=128, device='cuda:0'):
    ...
    classifier = HierarchicalClassifier(hierarchyL1, hierarchyL2, labelsL1, labelsL2, labelsL3, img_size=img_size, device=device)
...
def processFrame(frame, frame_time, frame_count, frames_after, useMotion, saveMovie, args, filename='', prevFilename=''):
        ...
        for box in boxes:
            ...
            if xyxy.size > 0: # Save object found
                ...
                if args.CSVformat == "tracking": # Format used for tracing insects
                    ...
                    input_variable = [args.camera, 1, timestamp_date_str, timestamp_time_str, prob, speciesName, speciesIdx+1, level, frame_count, x1, y1, x2, y2, saveFilename]
                ...
                if saveMovie:
                    ...
                    y = int(round(y1-20))
                    #MAMBO cv2.putText(frame, insectName, (x1,y), cv2.FONT_HERSHEY_SIMPLEX, 2, (255,255,255), 6, cv2.LINE_AA) # 1, 2 HD
    ...
    #MAMBO cv2.putText(frame, dateTimeStr, (40,60), cv2.FONT_HERSHEY_SIMPLEX, 2, (255,255,255), 6, cv2.LINE_AA) # (20,40) 1, 2 HD
...
if __name__=='__main__':
    ...
    #parser.add_argument('--video', default='./datasets/pi2_2025_04_06_23_11_00.mov') # 30 fps -> 10 fps (stride 3)
    parser.add_argument('--video', default='./datasets/Perchesbierg01_2022_07_11_14_18_06.mp4') # 30 fps -> 10 fps (stride 3)
    #parser.add_argument('--video', default='./datasets/beemoni12025_04_03_12_59_47.mp4') # 45 fps -> 5 fps (stride 5)
    ...
    parser.add_argument('--confidence', default='0.1', type=float) # insect3Color best F1-score 0.93
    ...
    parser.add_argument('--device', default='cuda:0') # used for GPU (cuda:X) or CPU (cpu) processing 
    parser.add_argument('--camera', default='CAM-1') # Overwritten by camera specified in image filename for time-lapse images
    parser.add_argument('--frame_stride', default='1', type=int) # for video, not used for images 2 fps
    ...
    if args.hierachical != '':
        ...
        modelClassifier = createHierarchicalClassifier(args.hierachical, args.labels, args.thresholds, 128, device=args.device)
    ...
    if video_path != '': # Process video file
        ...
        dateTimeStr = videoSplit[1] + videoSplit[2] + videoSplit[3] + videoSplit[4] + videoSplit[5] + "00" # Format: YYYYMMDDHHMMSS
        ...
        while cap.isOpened():
            ...
            if success: 
                if (frame_count % frame_stride == 0):    
                    ...
                    frame_time = frame_time + datetime.timedelta(seconds=1) # frame_stride
